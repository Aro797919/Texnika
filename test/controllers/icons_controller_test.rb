...
class IconsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create icon" do
    assert_difference("Icon.count") do
      post icons_url, params: { icon: { description: @icon.description, image_url: @icon.image_url, title: @icon.title } }
    end
  ...
  test "should update icon" do
    patch icon_url(@icon), params: { icon: { description: @icon.description, image_url: @icon.image_url, title: @icon.title } }
